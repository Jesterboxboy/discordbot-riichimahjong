...
class Tournament(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.auth_token=frey.login_through_pantheon(config.LOGIN_EMAIL,config.LOGIN_PASSWD)
    ...
    async def register_player(self,ctx, player_id,tourney_nr=config.TOURNEY):
        response = mimir.add_player(int(player_id),int(tourney_nr),self.auth_token)
        if (isinstance(response,TwirpServerException)):
           emoji = '\N{CROSS MARK}'
           await ctx.message.add_reaction(emoji)
           print(response)
